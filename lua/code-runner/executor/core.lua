...
function M.run_file(path)
    ...
    if cmd then
        M.last_run_path = path
        require("code-runner.terminal").run(cmd)
    end
end
...
function M.rerun_last_in_new_terminal()
    ...
    M.run_file_in_new_terminal(M.last_run_path)
end

---Reset internal state. Intended for use in tests only.
function M._reset_last_run_path()
    M.last_run_path = nil
end
