...
---Generate the command that creates a file
---@param file string
---@return string
function M.touch_command(file)
    if not utils.is_windows then return "touch " .. file end
    if utils.is_pwsh then
        return "New-Item -ItemType File -Force '" .. file .. "' | Out-Null"
    end
